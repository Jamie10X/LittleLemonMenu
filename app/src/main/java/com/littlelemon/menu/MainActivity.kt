...
class MainActivity : ComponentActivity() {
    ...
    private fun startProductActivity(productItem: ProductItem) {
        val intent = Intent(this, ProductActivity::class.java).also {
            it.putExtra(KEY_TITLE, productItem.title)
            it.putExtra(KEY_PRICE, productItem.price)
            it.putExtra(KEY_IMAGE, productItem.image)
            it.putExtra(KEY_CATEGORY, productItem.category)
        }
